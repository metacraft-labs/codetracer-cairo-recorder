...
fn test_recorded_trace_via_ct_print_json() {
    ...
    assert_eq!(
        ...
        "expected the call tree's root to open first; got {:?}",
        call_sequence
    );
    assert!(
        call_sequence[1].ends_with("::main"),
        "expected second call to be `main` (program entry point); got {:?}",
        call_sequence
    );
    assert!(
        call_sequence[2].ends_with("::compute"),
        "expected third call to be `compute` (called from main); got {:?}",
        call_sequence
    );
...
fn test_control_flow_test_via_ct_print_full() {
    ...
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(functions.len(), 6, "functions: {:?}", functions);
    ...
    // The call tree's root exits last and returns nothing: `start` opens
    // `<toplevel>` at depth 0 with no arguments, so it has no value to
    // return (trace-events.md, "Recorder Integration — Starting a
    // Recording").  It is pinned here and excluded from the Int-valued
    // sequence below rather than the sequence being loosened to admit it.
    let root_exit = doc["events"]
        .as_array()
        .expect("events array")
        .iter()
        .rev()
        .find(|e| e["kind"] == "call_exit")
        .expect("at least one call_exit");
    assert_eq!(root_exit["function"].as_str(), Some("<toplevel>"));
    assert_eq!(root_exit["return_value"]["kind"].as_str(), Some("Void"));
...
fn test_nested_calls_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "compute", "inner", "middle", "outer"]
    );
...
fn test_collections_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "compute", "array_total", "pair_sum"]
    );
...
fn test_error_paths_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "compute", "divide"]);
...
fn test_struct_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "compute"]);
...
fn test_panic_with_felt252_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "compute", "require_positive"]
    );
...
fn test_loop_while_for_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "compute", "loop_three", "loop_double"]
    );
...
fn test_storage_test_via_ct_print_full() {
    ...
    let events = doc["events"].as_array().expect("events array");
    // 6 steps + 6 call_entry + 6 call_exit + 3 io_events = 21 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
...
fn test_destructuring_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "use_pair"]);
...
fn test_snapshot_ref_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "compute", "read_only", "scale"]
    );
    ...
    let types: Vec<&str> = doc["types"]
        ...
        .filter_map(|v| v.as_str())
        .collect();
    // No `type_<id>` entry: a placeholder name minted for an id that was
    // never registered is a fabrication, and pinning it makes the
    // fabrication a requirement (conformance-testing.md, §"The shapes" —
    // "A placeholder that looks like data"; §"What this asks of a
    // conformance suite" #4 "Refuse rather than default").  The table is
    // exactly the types the recorder registered.
    assert_eq!(types, vec!["felt252", "Point{x,y}", "Ref"]);
...
fn test_numeric_widths_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "use_widths"]);
    ...
    let lookup_width_type = |name: &str| -> String {
        ...
        // A value's `type_id` is its record's position in the `types.dat`
        // interning table, so it indexes the table directly.  There is no
        // alias slot to step over: a `type_<id>` name minted for an id that
        // was never registered is a fabrication, not a table entry
        // (conformance-testing.md, §"The shapes" — "A placeholder that
        // looks like data").
        types
            .get(type_id)
            .map(|s| s.to_string())
...
fn test_array_operations_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "use_array"]);
...
fn test_event_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>".to_string(), "transfer".to_string()]
    );
    ...
    let events = doc["events"].as_array().expect("events array");
    // 3 steps + 2 call_entry + 2 call_exit + 1 io_event = 8 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    ...
    // Skip `<toplevel>`: `start` opens the call tree's root at depth 0
    // with no arguments (trace-events.md, "Recorder Integration —
    // Starting a Recording"), so the calldata lives on the frame after it.
    let call_entry = events
        .iter()
        .find(|e| e["kind"] == "call_entry" && e["function"] != "<toplevel>")
...
fn test_generic_function_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "pick_u32", "pick_u64"]);
    ...
    // conformance suite" #4 "Refuse rather than default").  The table is
    // exactly the types the recorder registered.
    assert_eq!(types, vec!["felt252", "u32", "u64"]);
    ...
    let lo_hi_emissions: Vec<(String, u64)> = events
        ...
        .collect();
    // The ids are the `types.dat` interning-table indices: felt252 = 0,
    // u32 = 1, u64 = 2.  There are no interleaved `type_<id>` entries to
    // step over — a placeholder minted for an unregistered id is a
    // fabrication, not a table entry (conformance-testing.md, §"The
    // shapes" — "A placeholder that looks like data").
    assert_eq!(
        lo_hi_emissions,
        vec![
            ("lo".to_string(), 1),
...
fn test_trait_impl_test_via_ct_print_full() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("<toplevel>".to_string(), "Void".to_string()),
        ]
    );
}
...
fn test_span_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "sum_span"]);
    ...
    assert_eq!(
        types,
        vec!["felt252", "Array<felt252>", "Span<felt252>", "u32"]
    );
...
fn test_match_pattern_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "run_all", "classify"]);
...
fn test_syscalls_test_via_ct_print_full() {
    ...
    let events = doc["events"].as_array().expect("events array");
    // 4 steps + 4 call_entry + 4 call_exit = 12 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    ...
    // conformance suite" #4 "Refuse rather than default").  The table is
    // exactly the types the recorder registered.
    assert_eq!(types, vec!["felt252", "Address", "u64"]);
    ...
    let exit_returns: Vec<(String, serde_json::Value)> = events
        ...
        .collect();
    assert_eq!(exit_returns.len(), 4, "expected 4 call_exit events");
    ...
    // return (trace-events.md, "Recorder Integration — Starting a
    // Recording").
    assert_eq!(exit_returns[3].0, "<toplevel>");
...
fn test_byte_array_short_string_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "make_greeting", "make_tag"]
    );
...
fn test_felt252_dict_test_via_ct_print_full() {
    ...
    // `start` registers before any function the recorder declares
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(bare_fns, vec!["<toplevel>", "main", "use_dict"]);
    ...
    // return (trace-events.md, "Recorder Integration — Starting a
    // Recording").
    assert_eq!(exit_returns[2].0, "<toplevel>");
...
fn test_hash_builtins_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "use_pedersen", "use_poseidon"]
    );
...
fn test_visibility_decorators_test_via_ct_print_full() {
    ...
    let self_kinds: Vec<(String, Option<bool>)> = events
        ...
        .collect();
    // `<toplevel>` leads: the call tree's root that `start` opens at
    // depth 0 carries no `self_kind` arg
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(
        self_kinds,
        vec![
            ("<toplevel>".to_string(), None),
    ...
    let arg_pairs: Vec<(String, Vec<(String, String)>)> = events
        ...
        .collect();
    // `<toplevel>` leads with an empty arg list: `start` opens the call
    // tree's root at depth 0 with no arguments
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(
        arg_pairs,
        vec![
            ("<toplevel>".to_string(), Vec::new()),
            (
...
fn test_component_test_via_ct_print_full() {
    ...
    let events = doc["events"].as_array().expect("events array");
    // 5 steps + 5 call_entry + 5 call_exit + 2 io_events = 17 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(events.len(), 17, "events.len()");
    ...
    let key_or_selector: Vec<(String, String)> = events
        ...
        .collect();
    // `<toplevel>` leads: the call tree's root that `start` opens at
    // depth 0 carries neither a `selector` nor a `key` arg
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(
        key_or_selector,
        vec![
            ("<toplevel>".to_string(), String::new()),
            (
...
fn test_closure_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "no_capture", "with_capture"]
    );
...
fn test_interface_dispatcher_test_via_ct_print_full() {
    ...
    let events = doc["events"].as_array().expect("events array");
    // 5 steps + 5 call_entry + 5 call_exit = 15 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    ...
    let dispatcher_args: Vec<(String, Option<String>)> = events
        ...
        .collect();
    // `<toplevel>` leads: the call tree's root that `start` opens at
    // depth 0 carries no `dispatcher_trait` arg
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    assert_eq!(
        dispatcher_args,
        vec![
...
fn test_ecdsa_test_via_ct_print_full() {
    ...
    assert_eq!(
        functions,
        vec!["<toplevel>", "0xec5a::check_ecdsa_signature"]
    );
    ...
    let events = doc["events"].as_array().expect("events array");
    // 3 steps + 3 call_entry + 3 call_exit = 9 events.
    // One of the call frames is `<toplevel>`, the call tree's root that
    // `start` opens at depth 0 and the recorder closes on exit
    // (trace-events.md, "Recorder Integration — Starting a Recording").
    ...
    let exit_returns: Vec<(String, serde_json::Value)> = events
        ...
        .collect();
    // Three exits: the two syscalls, then `<toplevel>` — the call tree's
    // root that `start` opens at depth 0 closes last
    // (trace-events.md, "Recorder Integration — Starting a Recording").
...
fn test_cairo_test_attribute_test_via_ct_print_full() {
    ...
    assert_eq!(
        bare_fns,
        vec!["<toplevel>", "main", "add_test", "sub_test", "mul_test"]
    );
    ...
    // return (trace-events.md, "Recorder Integration — Starting a
    // Recording").
    assert_eq!(exit_returns[4].0, "<toplevel>");
    assert_eq!(exit_returns[4].1["kind"].as_str(), Some("Void"));
}
